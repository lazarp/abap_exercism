...
CLASS ltcl_clock DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_clock_without_dates.
    METHODS:
...
CLASS ltcl_clock IMPLEMENTATION.
  METHOD on_the_hour.
    cut = NEW zcl_clock_without_dates( 8 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '08:00'
        act = cut->get( ) ).
  ENDMETHOD.
  METHOD past_the_hour.
    cut = NEW zcl_clock_without_dates( hours   = 11
                         minutes = 9 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '11:09'
        act = cut->get( ) ).
  ...
  METHOD adding_a_few_minutes.
    cut = NEW zcl_clock_without_dates( 10 ).
    cut->add( 3 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '10:03'
        act = cut->get( ) ).
  ...
  METHOD adding_zero_minutes.
    cut = NEW zcl_clock_without_dates( hours   = 6
                         minutes = 41 ).
    cut->add( 0 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '06:41'
  ...
  METHOD adding_more_than_two_hours_w_c.
    cut = NEW zcl_clock_without_dates( hours   = 0
                         minutes = 45 ).
    cut->add( 160 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '03:25'
  ...
  METHOD adding_more_than_two_days.
    cut = NEW zcl_clock_without_dates( hours   = 1
                         minutes = 1 ).
    cut->add( 3500 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '11:21'
  ...
  METHOD wrap_around_at_midnight.
    cut = NEW zcl_clock_without_dates( hours   = 23
                         minutes = 30 ).
    cut->add( 60 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '00:30'
  ...
  METHOD subtract_more_than_two_days.
    cut = NEW zcl_clock_without_dates( hours   = 2
                         minutes = 20 ).
    cut->sub( 3000 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '00:20'
  ...
  METHOD wrap_around_day.
    cut = NEW zcl_clock_without_dates( hours   = 5
                         minutes = 32 ).
    cut->add( 25 * 60 ).
    cl_abap_unit_assert=>assert_equals(
        exp = '06:32'
  ...
  METHOD equivalent_clocks.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_clock_without_dates( hours   = 15
                             minutes = 37 )->get( )
        act = NEW zcl_clock_without_dates( hours   = 15
                             minutes = 37 )->get( ) ).
  ENDMETHOD.
  METHOD inequivalent_clocks.
    cl_abap_unit_assert=>assert_differs(
        exp = NEW zcl_clock_without_dates( hours   = 1
                             minutes = 1 )->get( )
        act = NEW zcl_clock_without_dates( hours   = 18
                             minutes = 32 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_1.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_clock_without_dates( hours   = 3
                             minutes = 11 )->get( )
        act = NEW zcl_clock_without_dates( hours   = 99
                             minutes = 11 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_2.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_clock_without_dates( hours   = 22
                             minutes = 40 )->get( )
        act = NEW zcl_clock_without_dates( hours   = -2
                             minutes = 40 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_3.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_clock_without_dates( hours   = 17
                             minutes = 3 )->get( )
        act = NEW zcl_clock_without_dates( hours   = -31
                             minutes = 3 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_4.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_clock_without_dates( hours   = 2
                             minutes = 2 )->get( )
        act = NEW zcl_clock_without_dates( hours   = 2
                             minutes = 4322 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_5.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_clock_without_dates( hours   = 2
                             minutes = 40 )->get( )
        act = NEW zcl_clock_without_dates( hours   = 3
                             minutes = -20 )->get( ) ).
  ENDMETHOD.
  METHOD equivalent_clocks_6.
    cl_abap_unit_assert=>assert_equals(
        exp = NEW zcl_clock_without_dates( hours   = 7
                             minutes = 32 )->get( )
        act = NEW zcl_clock_without_dates( hours   = -12
                             minutes = -268 )->get( ) ).
  ENDMETHOD.
  METHOD hours_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '04:00'
        act = NEW zcl_clock_without_dates( 100 )->get( ) ).
  ENDMETHOD.
  METHOD minutes_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '04:43'
        act = NEW zcl_clock_without_dates( hours   = 0
                             minutes = 1723 )->get( ) ).
  ENDMETHOD.
  METHOD hours_and_minutes_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '00:00'
        act = NEW zcl_clock_without_dates( hours   = 72
                             minutes = 8640 )->get( ) ).
  ENDMETHOD.
  METHOD negative_hours_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '05:00'
        act = NEW zcl_clock_without_dates( -91 )->get( ) ).
  ENDMETHOD.
  METHOD negative_minutes_rollover.
    cl_abap_unit_assert=>assert_equals(
        exp = '16:40'
        act = NEW zcl_clock_without_dates( hours   = 1
                             minutes = -4820 )->get( ) ).
  ENDMETHOD.
  METHOD negative_hours_and_minutes.
    cl_abap_unit_assert=>assert_equals(
        exp = '22:10'
        act = NEW zcl_clock_without_dates( hours   = -121
                             minutes = -5810 )->get( ) ).
  ENDMETHOD.
ENDCLASS.
