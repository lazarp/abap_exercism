...
CLASS ltcl_itab_aggregation DEFINITION FOR TESTING RISK LEVEL HARMLESS DURATION SHORT FINAL.
  PRIVATE SECTION.
    DATA cut TYPE REF TO zcl_35_itab_aggregation.
    METHODS setup.
...
CLASS ltcl_itab_aggregation IMPLEMENTATION.
  METHOD setup.
    cut = NEW zcl_35_itab_aggregation( ).
  ENDMETHOD.
  METHOD empty.
    cl_abap_unit_assert=>assert_equals(
      act = cut->perform_aggregation( VALUE #( ) )
      exp = VALUE zcl_35_itab_aggregation=>aggregated_data( ) ).
  ENDMETHOD.
  METHOD one_row.
    DATA(actual_values) = VALUE zcl_35_itab_aggregation=>initial_numbers(
        ( group = 'A' number = 10 ) ).
    DATA(expected_values) = VALUE zcl_35_itab_aggregation=>aggregated_data(
